...
public class TXTLogWriter extends FileWriter {
    ...
    public void generateFile(int taxRegistrationNumber) throws IOException {
        ...
        if(getTaxpayerVariationTaxOnReceipts(taxRegistrationNumber) > 0)
            outputStream.println("Tax Increase: " + getTaxpayerVariationTaxOnReceipts(taxRegistrationNumber));
        else
            outputStream.println("Tax Decrease: " + getTaxpayerVariationTaxOnReceipts(taxRegistrationNumber));

        outputStream.println("Total Tax: " + getTaxpayerTotalTax(taxRegistrationNumber));
        outputStream.println("TotalReceiptsGathered: " + getTaxpayerTotalReceiptsGathered(taxRegistrationNumber));
        outputStream.println("Entertainment: " + getTaxpayerAmountOfReceiptKind(taxRegistrationNumber, ENTERTAINMENT));
        outputStream.println("Basic: " + getTaxpayerAmountOfReceiptKind(taxRegistrationNumber, BASIC));
        outputStream.println("Travel: " + getTaxpayerAmountOfReceiptKind(taxRegistrationNumber, TRAVEL));
        outputStream.println("Health: " + getTaxpayerAmountOfReceiptKind(taxRegistrationNumber, HEALTH));
